...
def send_whatsapp_otp_via_twilio(phone, otp, otp_type="signup"):
    ...
    Phone should be in format: +1234567890
    """
    if not twilio_client:
        print("[TWILIO] No Twilio credentials configured")
        return False
    ...
    message_text = f"Your E2EE Chat verification code is: {otp}\n\nThis code will expire in 10 minutes."
    ...
    try:
        ...
        print(f"[TWILIO] WhatsApp message sent to {phone}. SID: {message.sid}")
        return True
    except Exception as e:
        print(f"[TWILIO] Error sending WhatsApp message: {e}")
        traceback.print_exc()
        return False

def send_sms_otp_via_twilio(phone, otp, otp_type="signup"):
    """
    ...
    message_text = f"Your E2EE Chat code is: {otp}. It expires in 10 minutes."
...
def send_sms_otp(phone, otp, otp_type="signup"):
    """
    Send OTP via SMS. Tries Twilio first, then falls back to logging.
    """
    if twilio_client:
        try:
            sent = send_sms_otp_via_twilio(phone, otp, otp_type)
            if sent:
                return True
            else:
                print("[send_sms_otp] Twilio failed, falling back to log")
        except Exception as e:
            print("Error using Twilio SMS:", e)
            traceback.print_exc()

    print(f"[DEV-FALLBACK] SMS OTP for {phone}: {otp}")
    return True

def send_whatsapp_otp(phone, otp, otp_type="signup"):
    """
    Send OTP via WhatsApp. Tries Twilio first, then falls back to logging.
    """
    if twilio_client:
        try:
            sent = send_whatsapp_otp_via_twilio(phone, otp, otp_type)
            if sent:
                return True
            else:
                print("[send_whatsapp_otp] Twilio failed, falling back to log")
        except Exception as e:
            print("Error using Twilio WhatsApp:", e)
            traceback.print_exc()

    print(f"[DEV-FALLBACK] WhatsApp OTP for {phone}: {otp}")
    return True
...
def home():
    ...
    return "🔐 E2EE Secure Chat Server Running (with 2FA: Email/SMS/WhatsApp/TOTP; Postgres DB)"
...
def request_otp():
    ...
    print(f"[DEBUG] request-otp contact='{contact}'")
    is_email = '@' in contact
    print(f"[DEBUG] is_email={is_email}")
    if is_email:
        success = send_email_otp(contact, otp, "signup")
    else:
        success = send_whatsapp_otp(contact, otp, "signup")
...
def login():
    ...
    elif two_fa_method == "whatsapp":
        success = send_whatsapp_otp(contact, otp, "2fa")
    else:  # sms
